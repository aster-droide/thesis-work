...
def augment_audio(file_path_param, output_dir_param, pitch_shift_range=(-1, 1), time_stretch_range=(0.9, 1.1),
                  decibel_range=(-3, 3), num_augmented=5, original_sr=16000):
    """
    ...
    augmenter = am.Compose([
        am.PitchShift(min_semitones=pitch_shift_range[0], max_semitones=pitch_shift_range[1], p=0.3),
        am.TimeStretch(min_rate=time_stretch_range[0], max_rate=time_stretch_range[1], p=0.8),
        am.Gain(min_gain_in_db=decibel_range[0], max_gain_in_db=decibel_range[1], p=0.3)
    ])
...
output_dir = '/Users/astrid/Documents/Thesis/MEOWS/FreshMeowFolderFeb24/FINALFINALFINAL/AugmDs/AugmD22-files'
...
for fn in os.listdir(dataset_dir):
    if fn.endswith('.wav'):
        ...
        augment_audio(file_path, output_dir, num_augmented=num_aug)  # smaller time stretch range
